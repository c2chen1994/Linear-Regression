...
def regularized_linear_regression(X, y, lambd):
  ...
  #####################################################
  #        YOUR CODE HERE                             #
  # w = (Xt*X + lamda * I)-1 * Xt * y
  # np.eye(): 单位矩阵
  D = X.shape[1]
  I = np.eye(D)
  XT = np.transpose(X)
  w = np.dot(np.dot(np.linalg.inv(np.dot(XT, X) + lambd * I), XT), y)
  #####################################################    
  return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  for i in range(len(lambds)):
    lmd = lambds[i]
    w = regularized_linear_regression(Xtrain, ytrain, lmd)
...
def test_error(w, X, y):
  ...
  """
  # np.mean( , axis = 0/1) 0: 列平均值 1: 行平均值
  y_pred = np.dot(X, w)
